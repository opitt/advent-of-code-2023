...
def solve(input_lines):
    ...
    for y, line in enumerate(lines):
        ...
        if matches:
            gears.extend([Gear(y, m.start()) for m in matches])
    ...
    for gear in gears:
        parts = (
            [
                num.value
                for num in nums[gear.y - 1]
                if num.x1 - 1 <= gear.x <= num.x2 + 1
            ]
            + [
                num.value
                for num in nums[gear.y + 1]
            ...
            + [
                num.value
                for num in nums[gear.y]
                if num.x2 + 1 == gear.x or num.x1 - 1 == gear.x
            ]
        )
